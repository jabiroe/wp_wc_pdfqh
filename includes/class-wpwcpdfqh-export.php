 ...
 * PDF Export class
 */
if ( ! class_exists( 'WooCommerce_PDF_Quote_Handling_Export' ) ) {

	class WooCommerce_PDF_Quote_Handling_Export {
		...
		public function __construct() {					
			...
			$this->debug_settings = get_option('wp_wc_pdfqh_debug_settings');
			...
			$this->template_default_base_path = WooCommerce_PDF_Quote_Handling::$plugin_path . 'templates/' . $this->template_directory_name . '/';
			$this->template_default_base_uri = WooCommerce_PDF_Quote_Handling::$plugin_url . 'templates/' . $this->template_directory_name . '/';
			...
			add_action( 'wp_wc_pdfqh_processed_template_html', array($this, 'clear_page_number_styles' ), 10, 3 );
			add_action( 'wp_wc_pdfqh_after_dompdf_render', array($this, 'page_number_replacements' ), 9, 4 );

			add_action( 'wp_ajax_generate_wp_wc_pdfqh', array($this, 'generate_pdf_ajax' ));
			add_filter( 'woocommerce_email_attachments', array( $this, 'attach_pdf_to_email' ), 99, 3);
			add_filter( 'woocommerce_api_order_response', array( $this, 'woocommerce_api_quote_numer' ), 10, 2 );

			// check if an quote number filter has already been registered, if not, use settings
			if ( !has_filter( 'wp_wc_pdfqh_quote_number' ) ) {
				add_filter( 'wp_wc_pdfqh_quote_number', array( $this, 'format_quote_number' ), 20, 4 );
			}
			...
			if ( isset($this->debug_settings['html_output'])) {
				add_filter( 'wp_wc_pdfqh_output_html', '__return_true' );
				add_filter( 'wp_wc_pdfqh_use_path', '__return_false' );
			}

			if ( isset($this->template_settings['currency_font'])) {
				add_action( 'wp_wc_pdfqh_before_pdf', array($this, 'use_currency_font' ) );
			}
			...
			if ( class_exists('WC_Bundles') ) {
				add_filter( 'wp_wc_pdfqh_item_row_class', array( $this, 'add_product_bundles_classes' ), 10, 4 );
			}

			// WooCommerce Chained Products compatibility (add row classes)
			if ( class_exists('SA_WC_Chained_Products') ) {
				add_filter( 'wp_wc_pdfqh_item_row_class', array( $this, 'add_chained_product_class' ), 10, 4 );
			}
		...
		public function copy_fonts ( $path ) {
			$dompdf_font_dir = WooCommerce_PDF_Quote_Handling::$plugin_path . "lib/dompdf/lib/fonts/";
		...
		public function get_tmp_base () {
			...
			// 
			// May also be overridden by the wp_wc_pdfqh_tmp_path filter

			$upload_dir = wp_upload_dir();
			$upload_base = trailingslashit( $upload_dir['basedir'] );
			$tmp_base = trailingslashit( apply_filters( 'wp_wc_pdfqh_tmp_path', $upload_base . 'wp_wc_pdfqh/' ) );
			return $tmp_base;
		}
		...
		public function process_template( $template_type, $order_ids ) {
			...
			$order_ids = apply_filters( 'wp_wc_pdfqh_process_order_ids', $order_ids, $template_type );
			...
			do_action( 'wp_wc_pdfqh_process_template', $template_type );

			$output_html = array();
			foreach ($order_ids as $order_id) {
				$this->order = new WC_Order( $order_id );
				do_action( 'wp_wc_pdfqh_process_template_order', $template_type, $order_id );

				$template = $this->template_path . '/' . $template_type . '.php';
				$template = apply_filters( 'wp_wc_pdfqh_template_file', $template, $template_type, $this->order );
				...
				if ( $template_type == 'quote' ) {
					$this->set_quote_number( $order_id );
				}

				$output_html[$order_id] = $this->get_template($template);


				// store meta to be able to check if an quote for an order has been created already
				if ( $template_type == 'quote' ) {
					update_post_meta( $order_id, '_wc_pdfqh_quote_exists', 1 );
				}
			...
			if (apply_filters('wp_wc_pdfqh_output_html', false, $template_type) && apply_filters('wp_wc_pdfqh_print_html', false, $template_type)) {
		...
		public function generate_pdf( $template_type, $order_ids )	{
			$paper_size = apply_filters( 'wp_wc_pdfqh_paper_format', $this->template_settings['paper_size'], $template_type );
			$paper_orientation = apply_filters( 'wp_wc_pdfqh_paper_orientation', 'portrait', $template_type);

			do_action( 'wp_wc_pdfqh_before_pdf', $template_type );
			if ( !class_exists('DOMPDF') ) {
				...
				// than not checking...
				require_once( WooCommerce_PDF_Quote_Handling::$plugin_path . "lib/dompdf/dompdf_config.inc.php" );
			}

			$dompdf = new DOMPDF();
			$html = apply_filters( 'wp_wc_pdfqh_processed_template_html', $this->process_template( $template_type, $order_ids ), $template_type, $order_ids );
			$dompdf->load_html( $html );
			$dompdf->set_paper( $paper_size, $paper_orientation );
			$dompdf = apply_filters( 'wp_wc_pdfqh_before_dompdf_render', $dompdf, $html, $template_type, $order_ids );
			$dompdf->render();
			$dompdf = apply_filters( 'wp_wc_pdfqh_after_dompdf_render', $dompdf, $html, $template_type, $order_ids );
			do_action( 'wp_wc_pdfqh_after_pdf', $template_type );
		...
		public function generate_pdf_ajax() {
			...
			if( empty( $_GET['action'] ) || ! is_user_logged_in() || !check_admin_referer( $_GET['action'] ) ) {
				wp_die( __( 'You do not have sufficient permissions to access this page.', 'wp_wc_pdfqh' ) );
			}

			// Check if all parameters are set
			if( empty( $_GET['template_type'] ) || empty( $_GET['order_ids'] ) ) {
				wp_die( __( 'Some of the export parameters are missing.', 'wp_wc_pdfqh' ) );
			}
			...
			if( apply_filters( 'wp_wc_pdfqh_check_privs', !current_user_can( 'manage_woocommerce_orders' ) && !current_user_can( 'edit_shop_orders' ) && !isset( $_GET['my-account'] ), $order_ids ) ) {
				wp_die( __( 'You do not have sufficient permissions to access this page.', 'wp_wc_pdfqh' ) );
			}
			...
			if (apply_filters('wp_wc_pdfqh_output_html', false, $template_type)) {
				// Output html to browser for debug
				// NOTE! images will be loaded with the server path by default
				// use the wp_wc_pdfqh_use_path filter (return false) to change this to http urls
			...
			do_action( 'wp_wc_pdfqh_created_manually', $pdf, $filename );
		...
		public function build_filename( $template_type, $order_ids, $context ) {
			...
			switch ($template_type) {
				case 'quote':
					$name = _n( 'quote', 'quotes', $count, 'wp_wc_pdfqh' );
			...
			if ( $context == 'download' ) {
				$filename = apply_filters( 'wp_wc_pdfqh_bulk_filename', $filename, $order_ids, $name, $template_type );
			} elseif ( $context == 'attachment' ) {
				$filename = apply_filters( 'wp_wc_pdfqh_attachment_filename', $filename, $number, $order_ids[0] );	
			}

			// Filter filename (use this filter instead of the above legacy filters!)
			$filename = apply_filters( 'wp_wc_pdfqh_filename', $filename, $template_type, $order_ids, $context );
		...
		/**
		 * Attach quote to completed order or customer quote email
		 */
		public function attach_pdf_to_email ( $attachments, $status, $order ) {
			...
			$quote_allowed = isset($this->general_settings['email_pdf']) ? array_keys( $this->general_settings['email_pdf'] ) : array();
			$documents = array(
				'quote'	=>  apply_filters( 'wp_wc_pdfqh_email_allowed_statuses', $quote_allowed ), // Relevant (default) statuses: new_order, customer_invoice, customer_processing_order, customer_completed_order
			);
			$documents = apply_filters('wp_wc_pdfqh_attach_documents', $documents );
			...
			foreach ($documents as $template_type => $allowed_statuses ) {
				...
				// legacy filter, use wp_wc_pdfqh_custom_attachment_condition instead!
				$attach_quote = apply_filters('wp_wc_pdfqh_custom_email_condition', true, $order, $status );
				if ( $template_type == 'quote' && !$attach_quote ) {
					// don't attach quote, continue with other documents
					continue;
				}
				...
				$attach_document = apply_filters('wp_wc_pdfqh_custom_attachment_condition', true, $order, $status, $template_type );
				if( in_array( $status, $allowed_statuses ) && $attach_document ) {
					do_action( 'wp_wc_pdfqh_before_attachment_creation', $order, $status, $template_type );
					// create pdf data
					...
					do_action( 'wp_wc_pdfqh_email_attachment', $pdf_path, $template_type );
				}
			}
		...
		public function set_quote_number( $order_id ) {
			// first check: get quote number from post meta
			$quote_number = get_post_meta( $order_id, '_wc_pdfqh_quote_number', true );

			// add quote number if it doesn't exist
			if ( empty($quote_number) || !isset($quote_number) ) {
				global $wpdb;
				// making direct DB call to avoid caching issues
				$next_quote_number = $wpdb->get_var( $wpdb->prepare( "SELECT option_value FROM $wpdb->options WHERE option_name = %s LIMIT 1", 'wp_wc_pdfqh_next_quote_number' ) );
				$next_quote_number = apply_filters( 'wp_wc_pdfqh_next_quote_number', $next_quote_number, $order_id );

				if ( empty($next_quote_number) ) {
				...
				// reset quote number yearly
				if ( isset( $this->template_settings['yearly_reset_quote_number'] ) ) {
					$current_year = date("Y");
					$last_quote_year = get_option( 'wp_wc_pdfqh_last_quote_year' );
					// check first time use
					if ( empty( $last_quote_year ) ) {
						$last_quote_year = $current_year;
						update_option( 'wp_wc_pdfqh_last_quote_year', $current_year );
					}
					// check if we need to reset
					if ( $current_year != $last_quote_year ) {
						$quote_number = 1;
						update_option( 'wp_wc_pdfqh_last_quote_year', $current_year );
					}
				}
				...
				// quote number logging
				// $order_number = ltrim($this->order->get_order_number(), '#');
				// $this->log( $order_id, "Quote number {$quote_number} set for order {$order_number} (id {$order_id})" );

				update_post_meta($order_id, '_wc_pdfqh_quote_number', $quote_number);
				update_post_meta($order_id, '_wc_pdfqh_formatted_quote_number', $this->get_quote_number( $order_id ) );

				// increase next_order_number
				$update_args = array(
					'option_value'	=> $quote_number + 1,
					'autoload'		=> 'yes',
				);
				$result = $wpdb->update( $wpdb->options, $update_args, array( 'option_name' => 'wp_wc_pdfqh_next_quote_number' ) );
			}

			// store quote_number in class object
			$this->quote_number = $quote_number;

			// store quote number in _POST superglobal to prevent the number from being cleared in a save action
			// (http://wordpress.org/support/topic/customer-invoice-selection-from-order-detail-page-doesnt-record-invoice-id?replies=1)
			$_POST['_wc_pdfqh_quote_number'] = $quote_number;

			return $quote_number;
		}

		public function get_quote_number( $order_id ) {

			// get quote number from post meta
			if ( $quote_number = get_post_meta( $order_id, '_wc_pdfqh_quote_number', true ) ) {
				...
				return apply_filters( 'wp_wc_pdfqh_quote_number', $quote_number, $order_number, $order_id, $order_date );
			} else {
				// no quote number for this order
				return false;
			}
		}
		...
		/**
		 * Add quote number to WC REST API
		 */
		public function woocommerce_api_quote_numer ( $data, $order ) {
			if ( $quote_number = $this->get_quote_number( $order->id ) ) {
				$data['wp_wc_pdfqh_quote_number'] = $quote_number;
			} else {
				$data['wp_wc_pdfqh_quote_number'] = '';
			}
			return $data;
		}

		/**
		 * Reset quote  data for WooCommerce subscription renewal orders
		...
		public function woocommerce_subscriptions_renewal_order_created ( $renewal_order, $original_order, $product_id, $new_order_role ) {
			$this->reset_quote_data( $renewal_order->id );
		...
		public function reset_quote_data ( $order_id ) {
			// delete quote number, quote date & quote exists meta
			delete_post_meta( $order_id, '_wc_pdfqh_quote_number' );
			delete_post_meta( $order_id, '_wc_pdfqh_formatted_quote_number' );
			delete_post_meta( $order_id, '_wc_pdfqh_quote_date' );
			delete_post_meta( $order_id, '_wc_pdfqh_quote_exists' );
		}

		public function format_quote_number( $quote_number, $order_number, $order_id, $order_date ) {
			// get format settings
			$formats['prefix'] = isset($this->template_settings['quote_number_formatting_prefix'])?$this->template_settings['quote_number_formatting_prefix']:'';
			$formats['suffix'] = isset($this->template_settings['quote_number_formatting_suffix'])?$this->template_settings['quote_number_formatting_suffix']:'';
			$formats['padding'] = isset($this->template_settings['quote_number_formatting_padding'])?$this->template_settings['quote_number_formatting_padding']:'';
			...
			$quote_date = get_post_meta($order_id,'_wc_pdfqh_quote_date',true);
			$quote_date = empty($quote_date) ? current_time('mysql') : $quote_date;
			$quote_year = date_i18n( 'Y', strtotime( $quote_date ) );
			$quote_month = date_i18n( 'm', strtotime( $quote_date ) );
			$quote_day = date_i18n( 'd', strtotime( $quote_date ) );
			...
			foreach ($formats as $key => $value) {
				...
				$value = str_replace('[quote_year]', $quote_year, $value);
				$value = str_replace('[quote_month]', $quote_month, $value);
				$value = str_replace('[quote_day]', $quote_day, $value);
				$formats[$key] = $value;
			}

			// Padding
			if ( ctype_digit( (string)$formats['padding'] ) ) {
				$quote_number = sprintf('%0'.$formats['padding'].'d', $quote_number);
			}

			$formatted_quote_number = $formats['prefix'] . $quote_number . $formats['suffix'] ;

			return $formatted_quote_number;
		}
		...
		public function get_display_number( $order_id ) {
			...
			if ( isset($this->template_settings['display_number']) && $this->template_settings['display_number'] == 'quote_number' ) {
				// use quote number
				$display_number = $this->get_quote_number( $order_id );
				// die($display_number);
			} else {
				// use order number
		...
		public function get_order_items() {
			...
			if( sizeof( $items ) > 0 ) {
				foreach ( $items as $item_id => $item ) {
					...
					$data_list[$item_id] = apply_filters( 'wp_wc_pdfqh_order_item_data', $data, $this->order );
				}
			}

			return apply_filters( 'wp_wc_pdfqh_order_items_data', $data_list, $this->order );
		}
		...
		public function get_tax_rate( $tax_class, $line_total, $line_tax, $line_tax_data = '' ) {
			...
			if ( version_compare( WOOCOMMERCE_VERSION, '2.1' ) >= 0 && !apply_filters( 'wp_wc_pdfqh_calculate_tax_rate', false ) ) {
				// WC 2.1 or newer is used
		...
		public function get_thumbnail ( $product ) {
			...
			$size = apply_filters( 'wp_wc_pdfqh_thumbnail_size', 'shop_thumbnail' );
			...
			if (apply_filters('wp_wc_pdfqh_use_path', true) && !file_exists($thumbnail_path)) {
		...
		public function qtranslatex_filters() {
			$use_filters = array(
				'wp_wc_pdfqh_shop_name'		=> 20,
				'wp_wc_pdfqh_shop_address'	=> 20,
				'wp_wc_pdfqh_footer'			=> 20,
				'wp_wc_pdfqh_order_items'		=> 20,
				'wp_wc_pdfqh_payment_method'	=> 20,
				'wp_wc_pdfqh_shipping_method'	=> 20,
				'wp_wc_pdfqh_extra_1'			=> 20,
				'wp_wc_pdfqh_extra_2'			=> 20,
				'wp_wc_pdfqh_extra_3'			=> 20,
			);
		...
		public function use_currency_font ( $template_type ) {
			...
			add_action( 'wp_wc_pdfqh_custom_styles', array($this, 'currency_symbol_font_styles' ) );
		}
