...
$permissions = array(
	...
	'WC_PDFQH_ATTACHMENT_DIR'		=> array (
			...
			'value'				=> trailingslashit( $wp_wc_pdfqh->export->tmp_path( 'attachments' ) ),
			'status'			=> (is_writable( $wp_wc_pdfqh->export->tmp_path( 'attachments' ) ) ? "ok" : "failed"),			
			'status_message'	=> (is_writable( $wp_wc_pdfqh->export->tmp_path( 'attachments' ) ) ? "Writable" : "Not writable"),
		),
...
if ( isset($wp_wc_pdfqh->export->debug_settings['old_tmp']) ) {
	unset($permissions['WC_PDFQH_TEMP_DIR']);
}
...
<p>
The central temp folder (1.5+) is <code><?php echo $wp_wc_pdfqh->export->tmp_path(); ?></code>.
...
<code>wp_wc_pdfqh_tmp_path</code> filter. Make sure this folder is writable and that the
...
files (from <code><?php echo WooCommerce_PDF_Quote_Handling::$plugin_path . "lib/dompdf/lib/fonts/"; ?></code>)
